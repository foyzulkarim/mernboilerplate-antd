...
const {
  CustomerViewModel,
} = require("../models/view-models/customer-view-model");
...
const getAll = async () => {
  ...
  const viewModels = items.map((item) => CustomerViewModel.convert(item));
  return viewModels;
};
...
const getById = async (id) => {
  const model = await Model.findById(id);
  const viewModel = CustomerViewModel.convert(model);
  return viewModel;
};
